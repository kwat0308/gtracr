...
int main(int argc, char **argv) {
  ...
  void print_header();
  void print_result(double date, double d, double i, double h, double x,
                    double y, double z, double f);
  ...
  if ((argc == 2) &&
    ...
    printf("   Coord: D - Geodetic (WGS84 latitude and altitude above mean sea "
           "level)\n");
    printf("          C - Geocentric (spherical, altitude relative to Earth's "
           "center)\n");
  ...
  if ((argc == 5) && (*(args[2]) == 'f')) {
    ...
    fprintf(outfile, "Date Coord-System Altitude Latitude Longitude D_deg "
                     "D_min I_deg I_min H_nT X_nT Y_nT Z_nT F_nT dD_min dI_min "
                     "dH_nT dX_nT dY_nT dZ_nT dF_nT\n");
  } /* file option */
  ...
  while (again == 1) {
    if (coords_from_file) {
      ...
      if (read_flag == EOF)
        goto reached_EOF;
      fprintf(outfile, "%s %s %s %s %s ", args[2], args[3], args[4], args[5],
              args[6]);
      fflush(outfile);
      iline++;
    } /* coords_from_file */
    ...
    /* Note that there are no 'breaks' after the cases, so these are entry
     * points */
    switch (argc) {
    case 7:
    ...
    if (range == 2 && coords_from_file) {
      printf("Error in line %1d, date = %s: date ranges not allowed for file "
             "option\n\n",
             iline, args[2]);
      exit(2);
    }
    ...
    if (need_to_read_model) {
                                              ...
                                              * read to end of line or buffer */
      {
        ...
        /*       if (!strncmp(inbuff,"    ",4)){         /* If 1st 4 chars are
         * spaces */
    ...
    while ((range != 1) && (range != 2)) {
      printf("\nWould you like output for a single date or for a range of "
             "dates?\n");
    ...
    if (range == 1) {
      ...
      while ((sdate < minyr) || (sdate > maxyr)) {
        if (decyears == 1) {
          printf("\nEnter the decimal start date (%4.2f to %4.0f): ", minyr,
                 maxyr);
          safegets(inbuff, MAXREAD);
          sdate = atof(inbuff);
        } else {
      ...
      while ((edate <= sdate) || (edate > maxyr + 1)) {
        if (decyears == 1) {
          printf("\nEnter the decimal end date (%4.2f to %4.0f): ", sdate,
                 maxyr);
          safegets(inbuff, MAXREAD);
          edate = atof(inbuff);
        } else {
    ...
    if (coords_from_file && !arg_err && (igdgc != 1 && igdgc != 2)) {
      printf("\nError: unrecognized coordinate system %s in coordinate file "
             "line %1d\n\n",
             args[3], iline);
      arg_err = 1;
    }
    ...
    while ((igdgc != 1) && (igdgc != 2)) {
      ...
      printf("\n    1) Geodetic (WGS84 latitude and altitude above mean sea "
             "level)");
      printf("\n    2) Geocentric (spherical, altitude relative to Earth's "
             "center)\n");
      printf("\n                            ==> ");
      safegets(inbuff, MAXREAD);
      igdgc = atoi(inbuff);
    ...
    if (igdgc == 1) {
      if (coords_from_file && !arg_err && (units > 3 || units < 1)) {
        printf("\nError: unrecognized altitude units %s in coordinate file "
               "line %1d\n\n",
               args[4], iline);
        arg_err = 1;
      }
    ...
    while ((alt < minalt) || (alt > maxalt)) {
      if (igdgc == 2)
        printf("\n\nEnter geocentric altitude in km (%.2f to %.2f): ", minalt,
               maxalt);
      if (igdgc == 1 && units == 1)
        printf("\n\nEnter geodetic altitude above mean sea level in km (%.2f "
               "to %.2f): ",
               minalt, maxalt);
      if (igdgc == 1 && units == 2)
        printf("\n\nEnter geodetic altitude above mean sea level in meters "
               "(%.2f to %.2f): ",
               minalt, maxalt);
      if (igdgc == 1 && units == 3)
        printf("\n\nEnter geodetic altitude above mean sea level in feet (%.2f "
               "to %.2f): ",
               minalt, maxalt);
      safegets(inbuff, MAXREAD);
      alt = atof(inbuff);
    ...
    if (coords_from_file && !arg_err && (decdeg != 1 && decdeg != 2)) {
      printf("\nError: unrecognized lat %s or lon %s in coordinate file line "
             "%1d\n\n",
             args[5], args[6], iline);
      arg_err = 1;
    }
    ...
    if (decdeg == 1) {
      ...
      while ((latitude < -90) || (latitude > 90)) {
        printf("\n\nEnter the decimal latitude (-90 to 90) (- for Southern "
               "hemisphere).\n");
        safegets(inbuff, MAXREAD);
      ...
      if (coords_from_file && !arg_err &&
        ...
        printf("\nError: unrecognized longitude %s in coordinate file line "
               "%1d\n\n",
               args[6], iline);
        arg_err = 1;
      }
      ...
      while ((longitude < -180) || (longitude > 180)) {
        printf("\n\nEnter the decimal longitude (-180 to 180) (- for Western "
               "hemisphere).\n");
        safegets(inbuff, MAXREAD);
    ...
    // sdate: the starting date in decimal format

    if (max2[modelI] == 0) // if model isnt the last model in the file
    {
    ...
    if (coords_from_file) {
      print_result_file(outfile, d, i, h, x, y, z, f, ddot, idot, hdot, xdot,
                        ydot, zdot, fdot);
    } else {
      printf("\n\n\n  Model: %s \n", model[modelI]);
      if (decdeg == 1) {
      ...
      if (range == 1) {
        ...
        for (syr = sdate + step; syr < (edate + step); syr += step) {
          ...
          if (max2[modelI] == 0) /*If still not last element */
          {
            getshc(mdfile, 1, irec_pos[modelI], max1[modelI], 1);
            getshc(mdfile, 1, irec_pos[modelI + 1], max1[modelI + 1], 2);
            nmax = interpsh(syr, yrmin[modelI], max1[modelI], yrmin[modelI + 1],
                            max1[modelI + 1], 3);
            ...
            getshc(mdfile, 1, irec_pos[modelI], max1[modelI], 1);
            getshc(mdfile, 0, irec_pos[modelI], max2[modelI], 2);
            nmax = extrapsh(syr, epoch[modelI], max1[modelI], max2[modelI], 3);
            nmax =
                extrapsh(syr + 1, epoch[modelI], max1[modelI], max2[modelI], 4);
          }
          shval3(igdgc, latitude, longitude, alt, nmax, 3, IEXT, EXT_COEFF1,
                 EXT_COEFF2, EXT_COEFF3);
          dihf(3);
      ...
      if (warn_H) {
        printf("\nWarning: The horizontal field strength at this location is "
               "only %6.1f nT\n",
               warn_H_val);
        printf("         Compass readings have large uncertainties in areas "
               "where H is\n");
      ...
      if (warn_H_strong) {
        printf("\nWarning: The horizontal field strength at this location is "
               "only %6.1f nT\n",
               warn_H_strong_val);
        printf("         Compass readings have VERY LARGE uncertainties in "
               "areas where H is\n");
        printf("         smaller than 1000 nT\n\n");
      }
      if (warn_P) {
        printf("\nWarning: Location is at geographic pole where X, Y, and "
               "declination are not computed\n\n");
      }
    ...
    if (coords_from_file)
      ...
      if (argc == 7)
        again = 0; /* run command line only once */
      else
        do {
          printf("\n  Enter");
          printf("\n     0) to quit.");
...
reached_EOF:
  ...
  if (coords_from_file && !feof(coordfile) && arg_err)
    printf(
        "Terminated prematurely due to argument error in coordinate file\n\n");

  if (argc < 7) {
    printf("\nThe same result could have been generated with the following "
           "command:\n");
    ...
    if (range == 1) {
      ...
      if (decyears == 1) {
        ...
        printf("%d,%d,%d-%d,%d,%d-%4.2f ", isyear, ismonth, isday, ieyear,
               iemonth, ieday, step);
      }
    }
    if (igdgc == 1) {
    ...
    if (decdeg == 1) {
      ...
      printf("%d,%d,%d %d,%d,%d\n\n", ilat_deg, ilat_min, ilat_sec, ilon_deg,
             ilon_min, ilon_sec);
    }
  }
  return 0;
...
void print_header(void) {
  print_dashed_line();
  printf("   Date          D           I           H        X        Y        "
         "Z        F\n");
  printf("   (yr)      (deg min)   (deg min)     (nT)     (nT)     (nT)     "
         "(nT)     (nT)\n");
  return;
}
...
void print_result(double date, double d, double i, double h, double x, double y,
  ...
  if (my_isnan(d)) {
    if (my_isnan(x))
      printf("  %4.2f       NaN    %4dd %3.0fm  %8.1f      NaN      NaN %8.1f "
             "%8.1f\n",
             date, ideg, imin, h, z, f);
    else
      printf(
          "  %4.2f       NaN    %4dd %3.0fm  %8.1f %8.1f %8.1f %8.1f %8.1f\n",
...
void print_header_sv(void) {
  printf("   Date         dD          dI           dH       dX       dY       "
         "dZ       dF\n");
  printf("   (yr)      (min/yr)    (min/yr)    (nT/yr)  (nT/yr)  (nT/yr)  "
         "(nT/yr)  (nT/yr)\n");
} /* print_header_sv */
...
void print_result_sv(double date, double ddot, double idot, double hdot,
  ...
  if (my_isnan(ddot)) {
    if (my_isnan(xdot))
      printf("  %4.2f        NaN   %7.1f     %8.1f      NaN      NaN %8.1f "
             "%8.1f\n",
...
void print_result_file(FILE *outf, double d, double i, double h, double x,
  ...
  if (my_isnan(d)) {
    if (my_isnan(x))
      fprintf(outf,
              " NaN        %4dd %2.0fm  %8.1f      NaN      NaN %8.1f %8.1f",
              ideg, imin, h, z, f);
    else
      fprintf(outf, " NaN        %4dd %2.0fm  %8.1f %8.1f %8.1f %8.1f %8.1f",
              ideg, imin, h, x, y, z, f);
  } else
    fprintf(outf, " %4dd %2.0fm  %4dd %2.0fm  %8.1f %8.1f %8.1f %8.1f %8.1f",
            ddeg, dmin, ideg, imin, h, x, y, z, f);

  if (my_isnan(ddot)) {
    if (my_isnan(xdot))
      fprintf(outf,
              "      NaN  %7.1f     %8.1f      NaN      NaN %8.1f %8.1f\n",
              idot, hdot, zdot, fdot);
    else
...
double julday(month, day, year) int month;
int day;
int year;
{
...
int getshc(file, iflag, strec, nmax_of_gh, gh) char file[PATH];
int iflag;
long int strec;
int nmax_of_gh;
int gh;
...
int extrapsh(date, dte1, nmax1, nmax2, gh) double date;
double dte1;
int nmax1;
int nmax2;
int gh;
...
int interpsh(date, dte1, nmax1, dte2, nmax2, gh) double date;
double dte1;
int nmax1;
double dte2;
int nmax2;
...
int shval3(igdgc, flat, flon, elev, nmax, gh, iext, ext1, ext2, ext3) int igdgc;
double flat;
double flon;
double elev;
int nmax;
