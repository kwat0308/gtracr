...
// dvrdt
const double &RungeKutta::dvrdt(const double &r, const double &theta, const double &phi, const double &vr, const double &vtheta, const double &vphi)
{
    const double &dvrdt = ((coeff / gamma(vr, vtheta, vphi, r, theta)) * (vtheta * bfield->Bphi(r, theta, phi) -
                                                                          bfield->Btheta(r, theta, phi) * vphi)) +
                          ((vtheta * vtheta) / r) + ((vphi * vphi) / r);
    return dvrdt;
}

//dvthetadt
const double &RungeKutta::dvthetadt(const double &r, const double &theta, const double &phi, const double &vr, const double &vtheta, const double &vphi)
{
    const double &dvthetadt = ((coeff / gamma(vr, vtheta, vphi, r, theta)) * (vphi * bfield->Br(r, theta, phi) -
                                                                              bfield->Bphi(r, theta, phi) * vr)) -
                              ((vr * vtheta) / r) + ((vphi * vphi) / (r * tan(theta)));
    return dvthetadt;
}

//dvphidt
const double &RungeKutta::dvphidt(const double &r, const double &theta, const double &phi, const double &vr, const double &vtheta, const double &vphi)
{
    const double &dvphidt = ((coeff / gamma(vr, vtheta, vphi, r, theta)) * (vr * bfield->Btheta(r, theta, phi) -
                                                                            bfield->Br(r, theta, phi) * vtheta)) -
                            ((vr * vphi) / r) - ((vtheta * vphi) / (r * tan(theta)));
    return dvphidt;
}

const double &RungeKutta::velocity(const double &vr, const double &vtheta, const double &vphi, const double &r, const double &theta)
{
    const double &velocity = sqrt((vr * vr) + (r * r * vtheta * vtheta) + (r * r * sin(theta) * sin(theta) * vphi * vphi));
    return velocity;
}

const double &RungeKutta::gamma(const double &vr, const double &vtheta, const double &vphi, const double &r, const double &theta)
{
    const double &beta = velocity(vr, vtheta, vphi, r, theta) / constants::sc;
...
std::vector<double> RungeKutta::evaluate(std::vector<double> values)
{
    ...
    // double rk13 = sqrt((vr*vr) + (r * r* vth*vth) + (r * sin(th) * vph*r * sin(th) * vph));
    // std::cout << rk13 << std::endl;
    // std::cout << rk13 / constants::sc << std::endl;
    // std::cout << (rk13 / constants::sc)*(rk13 / constants::sc) << std::endl;
    // std::cout << 1. - (rk13 / constants::sc)*(rk13 / constants::sc) << std::endl;
    // std::cout << sqrt(1. - (rk13 / constants::sc)*(rk13 / constants::sc)) << std::endl;
    // std::cout << 1. / sqrt(1. - (rk13 / constants::sc)*(rk13 / constants::sc)) << std::endl;
    // double rk14 = 1. / (sqrt(1 - (rk13 / constants::sc)*(rk13 / constants::sc)));
    ...
    // double rk23 = sqrt((vr + 0.5 * a1)*(vr + 0.5 * a1) + (r + 0.5 * k1) *
    //                                     (vth + 0.5 * b1)*(r + 0.5 * k1) *
    //                                     (vth + 0.5 * b1) +
    //             (r + 0.5 * k1) * sin(th + 0.5 * l1) *
    //                 (vph + 0.5 * c1)*(r + 0.5 * k1) * sin(th + 0.5 * l1) *
    //                 (vph + 0.5 * c1));

    // std::cout << rk23 << std::endl;
    // std::cout << rk23 / constants::sc << std::endl;
    // std::cout << (rk23 / constants::sc)*(rk23 / constants::sc) << std::endl;
    // std::cout << 1. - (rk23 / constants::sc)*(rk23 / constants::sc) << std::endl;
    // std::cout << sqrt(1. - (rk23 / constants::sc)*(rk23 / constants::sc)) << std::endl;
    // std::cout << 1. / sqrt(1. - (rk23 / constants::sc)*(rk23 / constants::sc)) << std::endl;
    // double rk24 = 1. / (sqrt(1 - (rk23 / constants::sc)*(rk23 / constants::sc)));
    ...
    // double rk33 = sqrt((vr + 0.5 * a2)*(vr + 0.5 * a2) + ((r + 0.5 * k2) *
    //                                     (vth + 0.5 * b2)*(r + 0.5 * k2) *
    //                                     (vth + 0.5 * b2)) +
    //             ((r + 0.5 * k2) * sin(th + 0.5 *l2) *
    //                 (vph + 0.5 * c2)*(r + 0.5 * k2) * sin(th + 0.5 *l2) *
    //                 (vph + 0.5 * c2)));
    // double rk34 = 1. / (sqrt(1 - (rk33 / constants::sc)*(rk33 / constants::sc)));
    ...
    // std::cout << rk33 << std::endl;
    // std::cout << rk33 / constants::sc << std::endl;
    // std::cout << (rk33 / constants::sc)*(rk33 / constants::sc) << std::endl;
    // std::cout << 1. - (rk33 / constants::sc)*(rk33 / constants::sc) << std::endl;
    // std::cout << sqrt(1. - (rk33 / constants::sc)*(rk33 / constants::sc)) << std::endl;
    // std::cout << 1. / sqrt(1. - (rk33 / constants::sc)*(rk33 / constants::sc)) << std::endl;
    std::cout << k3 << l3 << m3 << a3 << b3 << c3 << std::endl;

    double k4 = h * (vr + a3);
    double l4 = h * ((vth + b3) / (r + k3));
    double m4 = h * ((vph + c3) / ((r + k3) * (sin(th + l3))));
    // double rk43 = sqrt((vr + a3)*(vr + a3) + ((r + k3) * (vth + b3)*(r + k3) * (vth + b3)) +
    //             ((r + k3) * sin(th + l3) * (vph + c3)*(r + k3) * sin(th + l3) * (vph + c3)));
    // double rk44 = 1. / (sqrt(1 - (rk43 / constants::sc)*(rk43 / constants::sc)));
    ...
    // std::cout << rk43 << std::endl;
    // std::cout << rk43 / constants::sc << std::endl;
    // std::cout << (rk43 / constants::sc)*(rk43 / constants::sc) << std::endl;
    // std::cout << 1. - (rk43 / constants::sc)*(rk43 / constants::sc) << std::endl;
    // std::cout << sqrt(1. - (rk43 / constants::sc)*(rk43 / constants::sc)) << std::endl;
    // std::cout << 1. / sqrt(1. - (rk43 / constants::sc)*(rk43 / constants::sc)) << std::endl;
