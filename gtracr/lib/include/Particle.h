...
public:
    // Default constructor, set to Proton (PID:2122, charge:+1e, mass:0.938272, label:p+)
    Particle();
    // Constructs a particle with some given name, pdgid, mass, charge, and label to associate the particle with.
    // The particle kinematics are set with the default energy of 1GeV. 
    Particle(const std::string & name, const int pdgid, const double &mass, const int charge, const std::string &label);
    // Constructs a particle with some given name, pdgid, mass, charge, and label to associate the particle with.
    // The particle kinematics can either be set with user inputs of rigidity or energy, but not both (or none), otherwise this will throw an error                    
    Particle(const std::string & name, const int pdgid, const double &mass, const int charge, const std::string &label, const double &energy, const double &rigidity);
    // destructor
    ~Particle();
