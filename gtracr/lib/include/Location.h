...
public:
    // Default Constructor
    // The default constructor is set with latitude, longitude, and altitude = 0. The name correlating to this
    // location is called "Null Island", and thus the name in this default constructor is "Null Island".
    Location();                      
    // Constructor
    // This constructs a Location object of any arbitrary latitude, longitude, and altitude as inputs. The name of the location 
    // is set to be "Default Name", for no other purposes other than to provide some input to the name variable.
    Location(const double & latitude, const double &longitude, const double &altitude);     
    // Constructor
    // This constructs a Location object of any arbitrary latitude, longitude, and altitude at some known location, provided 
    // with the variable name             
    Location(const std::string &name, const double &latitude, const double &longitude, const double &altitude); // with location name
    // destructors
    ~Location() {}
