...
convert_trajectory_to_cartesian(std::map<std::string, std::vector<double>> result_map)
{
    ...
    std::map<std::string, std::vector<double>> value_map = {
        {"t", time_arr}, {"x", x_arr}, {"y", y_arr}, {"z", z_arr}, {"px", px_arr}, {"py", py_arr}, {"pz", pz_arr}};

    return value_map;
} // namespace std::map<std::string,std::vector<double>>convert_trajectory_to_cartesian(std::map<std::string,std::vector<double>>result_map)

void plot_trajectory(std::map<std::string, std::vector<double>> trajectory_map)
{
...
int main()
{
    ...
    std::map<std::string, std::vector<double>> result_map = traj_tracer.evaluate_and_get_trajectories(initial_values);
    // here now we just evaluate and dont get the trajectory data
    // traj_tracer.evaluate(initial_values);

    std::map<std::string, std::vector<double>> cartesian_map = convert_trajectory_to_cartesian(result_map);
